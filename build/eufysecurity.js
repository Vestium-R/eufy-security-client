...
const security_mqtt_1 = require("./mqtt/security-mqtt");
...
class EufySecurity extends tiny_typed_emitter_1.TypedEmitter {
    ...
    async initSecurityMqtt(apiBase) {
        const hasSecurityMqttDevices = Object.values(this.devices).some((device) => device.usesSecurityMqtt());
        if (!hasSecurityMqttDevices) {
            logging_1.rootMainLogger.debug("No security MQTT locks found, skipping SecurityMQTT initialization");
            return;
        }
        const authToken = this.api.getToken();
        const userId = this.api.getPersistentData()?.user_id;
        if (!authToken || !userId) {
            logging_1.rootMainLogger.error("SecurityMQTT initialization failed: missing auth_token or user_id");
            return;
        }
        try {
            this.securityMqttService = new security_mqtt_1.SecurityMQTTService(authToken, userId, this.persistentData.openudid, this.config.country || "US");
            this.securityMqttService.on("connect", () => {
                logging_1.rootMainLogger.info("SecurityMQTT connected");
            ...
            this.securityMqttService.on("close", () => {
                logging_1.rootMainLogger.info("SecurityMQTT disconnected");
                for (const device of Object.values(this.devices)) {
                    if (device.usesSecurityMqtt()) {
                        const station = this.stations[device.getStationSerial()];
                        if (station) {
            ...
            this.securityMqttService.on("lock status", (deviceSN, locked, battery) => {
                this.getDevice(deviceSN)
                    .then((device) => {
                    device.updateProperty(types_1.PropertyName.DeviceLocked, locked);
                    // Battery is -1 when the heartbeat didn't include a battery TLV tag
                    if (battery >= 0) {
                        device.updateProperty(types_1.PropertyName.DeviceBattery, battery);
                    }
                })
                    .catch((err) => {
                    logging_1.rootMainLogger.debug("SecurityMQTT lock status update failed - device not found", {
                        deviceSN,
            ...
            this.securityMqttService.on("command response", (deviceSN, success) => {
                logging_1.rootMainLogger.debug("SecurityMQTT command response", { deviceSN, success });
            });
            await this.securityMqttService.connect(apiBase);
        }
        catch (err) {
            const error = (0, error_1.ensureError)(err);
            logging_1.rootMainLogger.error("SecurityMQTT initialization failed", { error: (0, utils_1.getError)(error) });
        }
    }
            ...
            if (stationsSNs.includes(hub.station_sn)) {
                ...
                promises.push(station.then((station) => {
                    try {
                        ...
                        station.on("security mqtt command", (_station, deviceSN, adminUserId, shortUserId, nickName, channel, sequence, lock, _property) => this.onSecurityMqttCommand(deviceSN, adminUserId, shortUserId, nickName, channel, sequence, lock));
    ...
    onStationConnect(station) {
        ...
        if (device_1.Device.usesSecurityMqtt(station.getDeviceType())) {
            return;
        }
    ...
    onSecurityMqttCommand(deviceSN, adminUserId, shortUserId, nickName, channel, sequence, lock) {
        if (!this.securityMqttService || !this.securityMqttService.isConnected()) {
            logging_1.rootMainLogger.error("SecurityMQTT not connected, cannot send lock command", { deviceSN });
            return;
        }
        ...
        this.securityMqttService
            ...
            if (success) {
                ...
                this.getDevice(deviceSN)
                    .then((device) => {
                    device.updateProperty(types_1.PropertyName.DeviceLocked, lock);
                })
                    .catch(() => {
            ...
            .catch((err) => {
            logging_1.rootMainLogger.error("SecurityMQTT lock command error", { error: (0, utils_1.getError)((0, error_1.ensureError)(err)) });
        });
    }
